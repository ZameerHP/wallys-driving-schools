...
async function runTests() {
  ...
  // Test 11: Operating hours closure respected (Sunday closed by default, bookable when enabled)
  const prevSettings = getInstructorSettings('wally');
  const initialSunday = await getAvailability({ date: '2026-10-18' }); // Sunday
  assert(!initialSunday.isOpen, 'Test 11A: Sunday when disabled is closed');

  // Temporarily enable Sunday
  saveInstructorSettings({
    operatingHours: {
      ...prevSettings.operatingHours,
      sunday: { ...prevSettings.operatingHours.sunday, enabled: true }
    }
  });
  const enabledSunday = await getAvailability({ date: '2026-10-18' });
  assert(enabledSunday.isOpen, 'Test 11B: Enabling Sunday dynamically makes it available');

  // Restore Sunday to disabled
  saveInstructorSettings({
