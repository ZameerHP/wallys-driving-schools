...
export function ManageBooking() {
  ...
  const handleSaveReschedule = async () => {
    ...
    try {
      ...
      setRescheduleSuccess(`Lesson successfully rescheduled to ${newDate} at ${newTime}! Your automatic email lesson reminder has been updated for 2 hours prior to your new lesson.`);
    } catch (err: any) {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      if (isRefunded) {
        setCancelSuccess(`Booking #${selectedBooking.ref} has been cancelled and any scheduled lesson reminder emails have been cancelled. Because your lesson was cancelled more than 24 hours in advance, a full refund of $${selectedBooking.packagePrice.toFixed(2)} AUD has been processed back to your original payment method via Stripe.`);
      } else if (isPaid) {
        setCancelSuccess(`Booking #${selectedBooking.ref} has been cancelled and any scheduled lesson reminder emails have been cancelled. Note: Because your lesson is scheduled within 24 hours, per driving school policy it is classified as a late cancellation without an automatic refund. If you need assistance, please contact Wally.`);
      } else {
        setCancelSuccess(`Booking #${selectedBooking.ref} has been cancelled, any scheduled lesson reminder emails have been cancelled, and your scheduled time slot has been released.`);
      }
    } catch (err: any) {
        ...
        {selectedBooking ? (
          ...
          >
            ...
            <div className="py-6 space-y-4">
              ...
              {/* Automatic Resend Email Lesson Reminder Information */}
              {selectedBooking.status !== 'Cancelled' && (
                <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-2xl p-3.5 flex items-start gap-3 text-xs text-emerald-900 mt-2">
                  ...
                  <div>
                    <span className="font-bold text-emerald-950 block">
                      Automatic Email Lesson Reminder Active
                    </span>
                    <p className="text-emerald-800/90 mt-0.5 leading-relaxed text-[11px]">
                      A reminder will be sent to your email (<strong>{selectedBooking.email}</strong>) automatically 2 hours before your lesson start time ({selectedBooking.time.split('-')[0]?.trim() || selectedBooking.time}).
                    </p>
                  </div>
                </div>
              )}
